...
for i = tspan(1) : tspan(s)% Initialize for loop to run for the domain
    ...
    while err < es && iter < maxit% embeded while loop runs until approx error condition is met
        ...
        guess1 = guess2 + h * dydt(i,guess2);%Use Euler's method to find y at i + 1
        sloperight = dydt(i+1,guess1);
        slopecorrect = (y0 + (h/2) * (slopeleft + sloperight)); %find the average value between y(i) and y(i+1) (y0 +
        %(h/2)*(slopeleft + sloperight)
        guess2 =  guess2 + h * dydt(i,guess2);%use corrector value to get a better estimate at y(i+1)
